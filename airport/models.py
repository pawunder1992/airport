    ...
    def validate_flight_time(departure_time, arrival_time, error_to_raise):
        if departure_time > arrival_time:
            raise error_to_raise(
                "departure_time can`t be greater than arrival_time"
            )
...
class Ticket(models.Model):
    ...
    def validate_seat(seat, airplane_seats, error_to_raise):
        ...
        if seat not in alphabet[:airplane_seats]:
            raise error_to_raise(
                {
                    "seat": f"seat must be a letter in range [A, {alphabet[airplane_seats-1]}], not {seat}"
                }
            )
    ...
    def validate_row(row, airplane_rows, error_to_raise):
        if not (1 <= row <= airplane_rows):
            raise error_to_raise(
                {
                    "row": f"row must be in range [1, {airplane_rows}], not {row}"
                }
            )

    def clean(self):
        Ticket.validate_seat(
            self.seat, self.flight.airplane.seats_in_row, ValueError
        )
